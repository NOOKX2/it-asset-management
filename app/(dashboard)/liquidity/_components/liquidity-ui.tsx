...
export function AssetTypeBadge({ securityType }: { securityType: string }) {
  ...
  const isFund = lower.includes("fund") || lower.includes("กองทุน");
  const isStock = lower.includes("stock") || lower.includes("หุ้น");
  const isBond = lower.includes("bond") || lower.includes("พันธบัตร");
  const isCash =
    lower.includes("cash") || lower.includes("เงินสด") || lower.includes("เงินฝาก");
  const isLoan =
    lower.includes("loan") ||
    lower.includes("receivable") ||
    lower.includes("เงินให้กู้");
  ...
  if (isGold) {
    ...
    bg = "bg-orange-50 text-[var(--primary-green)]";
    icon = (
      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path
          strokeLinecap="round"
